...
public final class CompositeDirectiveRegistry implements DirectiveRegistry {
  ...
  @Nullable
  @Override
  public DirectiveInfo get(String context, String directive) throws DirectiveLoadException {
    for (int i = 0; i < registries.length; ++i) {
      DirectiveInfo info = registries[i].get(context, directive);
      if (info != null) {
        return info;
      }
    }
  ...
  @Override
  public void reload(String context) throws DirectiveLoadException {
    for (int idx = 0; idx < registries.length; ++idx) {
      registries[idx].reload(context);
    }
  }
